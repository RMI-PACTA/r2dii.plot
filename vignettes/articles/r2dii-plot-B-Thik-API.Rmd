...
For the "thick API", we are experimenting with three ways to plot the prepared
data. The main difference between them is if and how they allow recoding the
values of `line_name`, which become the labels of the plot legend.

Regardless of the version, you'll need to subset the data to meet the
requirements of the `plot_*()` functions. If you don't get it right, the error
should guide you.
...
* `plot_timelineA()` defaults to recoding `line_name` to title case, and allows custom recoding via a data frame passed to the argument `specs`.
...
* `plot_timelineC()` defaults to recoding `line_name` to title case, and allows recoding via the argument `recode`, which can take multiple forms, including a function 
