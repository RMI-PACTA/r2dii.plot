...
trajectory and two types of functions: `qplot_*()` for standardized plotting and 
`plot_*()`leaving more space for customization. 

For each kind, you'll need to subset the specific rows you want to
plot. For details see the documented "Requirements" of the argument `data` of
each `plot_*()` or `qplot_*()` function (e.g. see `?plot_emission_intensity`). 
If you forget to meet the `data` requirements the error message should guide you.
...
users who are interested in standard PACTA visualizations. Their common characteristics
are:
...
The `plot_*()` functions plot the data as they are and allow you to add your own customizations by modifying the input (for example by adding a column 'label' or
'label_tech' for changing the data labels) or using `ggplot2` functions to change 
...
You can polish your plot by modifying the input and the `ggplot` object. 
...
Example of customized `plot_trajectory()`.
...
Example of customized `plot_techmix()`.
