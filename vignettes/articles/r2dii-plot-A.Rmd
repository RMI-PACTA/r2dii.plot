...
market_share %>%
  ...
  plot_techmixX() + labs(title = "power")
```

How would you create one plot for every sector? One way is to wrap the code
in a helper function and make `sector` and argument. Then you can iterate over
each sector with `lapply()`.

```{r}
plot_sector_techmix <- function(x) {
  chosen_metrics <- c("projected", "corporate_economy", "target_sds")
  ...
  market_share %>%
    ...
    plot_techmixX() + labs(title = x)
}

(sectors <- unique(market_share$sector))
lapply(sectors, plot_sector_techmix)
```
