...
The `plot_*()` functions show the data as they are and allow you to add your own
customization through their parameters, by modifying the input (for example, by
...
properties of `qplot_*` and modify others (see the documentation of each
function for details). For example, here is the output of
`plot_emission_intensity()` with parameters replicating the behavior of
`qplot_emission_intensity()`. (Note that the plot labels were added manually
using the `ggplot2::labs()` function and not using function parameters.)
