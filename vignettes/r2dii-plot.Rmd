...
```{r}
# focus on production plans for technologies in the 'power', globally, for 5
# years since 2020
data <- market_share %>%
  filter(
    metric == "projected",
...
```{r}
# focus on planned production of renewable technologies, for portfolio
# ("projected") and scenarios, over time
data <- market_share %>%
  filter(
...
```{r}
# focus on change in production plans, per sector, for 5 years since 2020
data <- market_share %>%
  filter(
    metric == "projected",
    region == "global",
...
```{r}
# plot change in production plans, for technologies in the 'power' sector, for 5
# years since 2020
data <- market_share %>%
  filter(
    metric == "projected",
