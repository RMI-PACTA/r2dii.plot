...
2) Use `ggplot2` functions,
3) Modify the input `data`. 

The following sub sections show the three strategies applied to different functions 
of the `plot_*()` family.

### Plot customization strategies 1 and 2: parameters and ggplot2 functions

We will show how you might customize a plot using strategy 3 and 2 based on `plot_emission_intensity()`. The basic output of the function looks rather 
unappealing.
...
You can use parameters of `plot_emission_intensity()` to replicate features of `qplot_emission_intensity()`(strategy 1), and add plot labels with 
`ggplot2::labs()` (strategy 2).
...
### Plot customization strategies 3 and 2: input data and ggplot2 functions

You can also polish your plot by modifying the input `data` (strategy 3), for 
example: 
...
And by modifying output `ggplot` object (strategy 2), for example:
