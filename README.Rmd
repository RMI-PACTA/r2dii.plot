...
pull_title <- function(name, db = docs_df("r2dii.ggplot")) {
  subset <- db$name == name
  out <- db$title[subset][[1]]
  tolower(out)
}

docs_df <- function(package) {
  db <- utils::hsearch_db(package, lib.loc = find_pkg(package))[["Base"]]
  names(db) <- tolower(names(db))
  db
}

find_pkg <- function(pkg) {
