...
* `market_share` dataset imitating the output of 'r2dii.analysis::target_market_share()'.
...
* `prep_trajectory()` and `prep_trajectoryB()`: prepares pre-processed data for plotting a trajectory chart.
...
* `prep_techmix()` prepares pre-processed data for plotting a tech-mix chart.
...
* `prep_techmixB()` is an alternative with fewer arguments. It requires more
work upfront but if you forget its requirements you will get error messages that
we hope will help you fix the problem.
...
* `prep_timeline()` prepare the output of 'r2dii.analysis::target_sda()' for 'plot_timeline()'.
...
* `timeline_specs()` creates the default specs data frame for 'plot_timelinea()'.
